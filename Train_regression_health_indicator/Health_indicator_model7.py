...
def test_part(label_size,time_step_size,input_vec_size,test_size,mean_diff,var_diff,model_num):
    ...
    for i in range(test_size):
        ...
        print "prediction for HI : " + str(prediction_label[i]) + " //  real HI : " + str(label[i] )
    ...
    save_result_file_name = '../Test_result_saved/Model'+str(model_num)+'/mean_diff_HI.pkl'
    try:
    ...
    save_result_file_name = '../Test_result_saved/Model'+str(model_num)+'/var_diff_HI.pkl'
    try:
        os.remove(save_result_file_name)
    except OSError:
        print 'no saved file  ---  creating'
...
def draw_graphe_part(time_step_size,input_vec_size,label_size,model_num):
    ...
    if (step+1)%1==0 and step>0:
        ...
        for file_itr in range(35):
            ...
            save_result_file_name = '../Graphe_result_saved/Model'+str(model_num)+'/File'+ str(file + 1) +'/Step'+str(step+1)+'_prediction_HI.pkl'
            with open(save_result_file_name,'wb') as result_to_save:
...
with tf.Session() as sess:
    ...
    for step in range(learning_steps):
        ...
        # save model
        save_path = saver.save(sess,'../Model_saved/Model'+str(model_num)+'/Model'+str(model_num)+'_HI.tfmodel')