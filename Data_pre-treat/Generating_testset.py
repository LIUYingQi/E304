...
try:
    os.remove('../Test_data_set/TestsetInfo.pkl')
except OSError:
...
for i in range(test_set_volume):
    test_set_file_name = '../Test_data_set/TestLabel' + str(i+1) + '.pkl'
    try:
...
for i in range(test_set_volume):
    test_set_file_name = '../Test_data_set/Testset' + str(i+1) + '.pkl'
    try:
        os.remove(test_set_file_name)
    except OSError:
        print 'no such files  Test set  ----   continue  '
...
for test_num in range(test_set_volume):
    ...
    test_set_file_name = '../Test_data_set/Testset' + str(test_num+1) + '.pkl'
    ...
    # save label
    test_set_file_name = '../Test_data_set/TestLabel' + str(test_num + 1) + '.pkl'
    with open(test_set_file_name, 'wb') as dataset_label_to_save:
        pickle.dump(np.array([label_to_add]), dataset_label_to_save)

with open('../Test_data_set/TestsetInfo.pkl','wb') as dataset_info_to_save:
